...
class InputAgent extends EventEmitter {
  ...
  _proxyHandler = {
    get: (target, property, receiver) => {
      ...
      switch (typeof result) {
        case "function":
          return (...argv) => {
            const str = result.apply(target, argv);
            if(typeof str !== "string") {
              this.emit("error", new Error(
                `apply InputAgent#erase.${property}: Unexpected type ${typeof str} received.`
              ));
              return str;
            }

            return new Promise((resolve, reject) => {
              this.stdout.write(str, err => err ? reject(err) : resolve());
            });
          };
        case "string":
          return new Promise((resolve, reject) => {
            this.stdout.write(result, err => err ? reject(err) : resolve());
          });
        default:
          this.emit("error", new Error(
            `get InputAgent#erase.${property}: Unexpected type ${typeof result} received.`
          ));
          return result;
      }
    }
  ...
  erase  = new Proxy(ansi.erase,  this._proxyHandler);
  cursor = new Proxy(ansi.cursor, this._proxyHandler);
}
